...
void svc_handler_c(uint32_t asm_lr,uint32_t asm_mspbackup)
{
	printf("[SVC Handler] LR: 0x%X\r\n", (unsigned int)asm_lr);
	printf("[SVC Handler] MSP Backup: 0x%X \r\n", (unsigned int)asm_mspbackup);
	...
	if ((asm_lr & 0b100)>>1) //Test bit 2 of EXC_RETURN
	{
	...
	{
		stack_frame_ptr = (uint32_t *)asm_mspbackup; //if 0, stacking used MSP
		printf("[SVC Handler] Stacking used MSP: 0x%X \r\n\n", (unsigned int)stack_frame_ptr);
	}
	
	uint32_t stacked_r0 = (uint32_t *)stack_frame_ptr[0];
	uint32_t stacked_r1 = (uint32_t *)stack_frame_ptr[1];
